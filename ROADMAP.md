...
optional per-entry `bonusPercent` / `exactAmountNim`, most generous match
wins), `whitelist-bonus` adjustment in
[apps/server/src/rewards/automatic.ts](apps/server/src/rewards/automatic.ts)
...
and 3 MCP tools (`faucet.reward_whitelist_{add,remove,list}`).
