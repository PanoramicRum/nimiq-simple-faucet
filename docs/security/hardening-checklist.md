...
Live checklist mirroring milestone M6.2 in the plan. Copy-paste any
subsection into a PR comment or review thread. Tick items as they ship on
`main`; leave them unchecked until the code is merged (not just designed).

## Transport + headers

- [ ] `@fastify/helmet` registered with strict CSP for `/admin` (`default-src 'self'`)
...
- [ ] TLS required in production; server refuses to start on plain HTTP unless `FAUCET_DEV=1`
...
- [ ] Global per-IP per-minute limit via `@fastify/rate-limit` (already shipped)
...
- [ ] Argon2id parameters: `memoryCost=64 MiB`, `timeCost=3`, `parallelism=4` (review vs. current OWASP guidance)
- [ ] HMAC replay window: 5 minutes (shipped); nonce cache cleanup audited
...
- [ ] Log scrubber strips secrets, API keys, cookies, TOTP codes, HMAC signatures
- [ ] Every denied claim logs signals to `audit_log` (not to request logs)
- [ ] Structured error IDs returned so support can cross-reference without exposing internals
